...
[![CI](https://github.com/datascale-ai/datasmith/actions/workflows/ci.yml/badge.svg)](https://github.com/datascale-ai/datasmith/actions/workflows/ci.yml)
[![PyPI](https://img.shields.io/pypi/v/datasmith)](https://pypi.org/project/datasmith/)
...
- [DataSmith](#datasmith)
  - [目录](#目录)
  - [DataSmith 是什么？](#datasmith-是什么)
  - [效果示例](#效果示例)
  - [安装](#安装)
  - [快速开始](#快速开始)
    - [立即试用（无需 API Key）](#立即试用无需-api-key)
    - [使用云端 API（OpenAI / DeepSeek）](#使用云端-apiopenai--deepseek)
    - [使用本地 vLLM 服务器](#使用本地-vllm-服务器)
  - [工作原理](#工作原理)
  - [CLI 命令](#cli-命令)
  - [支持的后端](#支持的后端)
  - [配置参数说明](#配置参数说明)
  ...
  - [性能基准](#性能基准)
    - [吞吐量](#吞吐量)
    - [容错能力（30% 随机故障注入）](#容错能力30-随机故障注入)
    - [云 API 限速合规（qwen-plus，RPM=120）](#云-api-限速合规qwen-plusrpm120)
    - [分布式扩展（RayActor，4× workers）](#分布式扩展rayactor4-workers)
  - [文档](#文档)
  - [参与贡献](#参与贡献)
  - [引用](#引用)
  - [许可证](#许可证)
...
DataSmith 是一个生产级异步流水线，自动处理速率限制、重试、检查点和质量过滤——让你专注于生成*什么*数据，而不是*如何*生成。无论你使用云端 API 还是本地 vLLM 服务器，它的工作方式完全一致。
...
以下是 DataSmith 使用 `evol-instruct`（深度为 2）对种子指令的处理结果：
...
从源码安装：
```bash
git clone https://github.com/datascale-ai/datasmith.git
cd datasmith
pip install -e ".[dev]"
```
...
**第三步**，运行：
```bash
python -m datasmith run config.yaml --backend fake
```
...
sink:
  path: ./output.jsonl
  checkpoint_dir: ./.datasmith_runs   # 崩溃安全：用同一命令重启即可续跑
...
| 命令 | 说明 |
|------|------|
| `datasmith run config.yaml` | 运行合成流水线 |
| `datasmith run config.yaml --dry-run` | 验证配置但不运行 |
| `datasmith assess output.jsonl` | 生成数据质量报告（JSON / HTML） |
| `datasmith benchmark config.yaml` | 运行 MT-Bench / IF-Eval 精简评测 |
| `datasmith status .datasmith_runs` | 查看检查点进度 |
| `datasmith inspect output.jsonl` | 打印输出统计信息 |
...
| `sink.checkpoint_dir` | `.datasmith_runs` | WAL 检查点目录 |
...
| 数据质量 CLI | `datasmith assess` 生成结构化质量报告 |
...
| Naive Async（公平基线）| 1,572 vs 1,402 条/分 | **+12.1%**，p=0.040，Cohen's d=2.79 |
| Distilabel | 3,188 vs 1,265 条/分 | **+152%**（2.52×） |
...
如果你在研究中使用了 DataSmith，请引用：

```bibtex
@software{datasmith2026,
  title  = {DataSmith: High-Concurrency Async Pipeline for LLM Training Data Synthesis},
  year   = {2026},
  url    = {https://github.com/datascale-ai/datasmith}
}
```
...
MIT © [DataSmith Contributors](https://github.com/datascale-ai/datasmith/graphs/contributors)
