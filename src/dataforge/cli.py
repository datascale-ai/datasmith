...
def _setup_logging(level: str, log_file: str | None = None) -> None:
    """Configure root logging for the datasmith package."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="datasmith",
        description="DataSmith — high-concurrency LLM data synthesis pipeline",
    )
    parser.add_argument(
        "--log-level",
        default="WARNING",
...
def _run(config_path: str, dry_run: bool = False) -> None:
    ...
    from datasmith.config import build_pipeline, load_config
...
def _validate(config_path: str) -> None:
    ...
    from datasmith.config import load_config
...
def _shard(args: argparse.Namespace) -> None:
    from datasmith.distributed.shard import (
...
def _coordinator(args: argparse.Namespace) -> None:
    ...
    from datasmith.config import load_config
    from datasmith.distributed.coordinator import Coordinator
...
def _worker(args: argparse.Namespace) -> None:
    ...
    from datasmith.config import build_pipeline, load_config
    from datasmith.distributed.worker import DistributedWorker
