"""DataSmith Ray Actor distributed backend.

Wraps DataSmith Pipeline inside a Ray remote Actor so that:
  ...
  - All DataSmith features work: RetryEngine, EvolInstruct, LLMJudge, prefix scheduling
...
Architecture::
    ...
    RayOrchestrator.run(input_path, output_path, ...)
      ...
      ├── DataSmithActor × N  (Ray schedules across nodes)
...
Usage::
    ...
    from datasmith.distributed.ray_actor import RayOrchestrator
...
def _make_actor_class():
    """Return the DataSmithActor class (defined inside Ray context)."""
    try:
        import ray
    ...
    class DataSmithActor:
        """Ray Actor wrapping a DataSmith Pipeline.
...
class RayOrchestrator:
    """Orchestrates N DataSmithActors across a Ray cluster."""
    ...
    def run(
        ...
        *,
        ...
        """Run DataSmith pipeline across N Ray Actors.
