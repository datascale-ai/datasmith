# src/datasmith/registry.py
"""Plugin registry for strategies and evaluators.

Allows dynamic registration via decorators::

    from datasmith.registry import register_strategy, register_evaluator
...
def _load_entry_points() -> None:
    """Discover third-party plugins via ``datasmith.strategies`` and
    ``datasmith.evaluators`` entry-point groups.

    Each entry point should point to a module that registers its classes
    by importing from ``datasmith.registry`` at module scope.
    """
    try:
