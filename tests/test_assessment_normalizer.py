...
from datasmith.assessment.normalizer import detect_source_format, normalize_row


def test_detect_source_format_variants():
    assert detect_source_format({"seed_data": {}, "synthetic_data": {}}) == "datasmith_jsonl"
    assert detect_source_format({"instruction": "q", "response": "a"}) == "instruction_response_jsonl"


def test_normalize_datasmith_row():
