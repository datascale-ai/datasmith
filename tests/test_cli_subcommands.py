...
def test_validate_valid_config(capsys):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        from datasmith.cli import _validate
...
def test_status_with_checkpoint(capsys):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        from datasmith.cli import _status
...
def test_inspect_output_file(capsys):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        from datasmith.cli import _inspect
...
def test_version(capsys):
    from datasmith.cli import _version

    _version()
    output = capsys.readouterr().out
    assert "datasmith" in output
