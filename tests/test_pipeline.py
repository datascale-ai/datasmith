...
from datasmith.pipeline import Pipeline, _plan_zero_overhead_batch
...
async def test_pipeline_burst_prefers_checkpoint_done_id_snapshot(monkeypatch):
    ...
    monkeypatch.setattr("datasmith.pipeline.CheckpointManager", FakeCheckpoint)
...
async def test_pipeline_burst_windowed_gather(monkeypatch):
    ...
    monkeypatch.setattr("datasmith.pipeline.asyncio.gather", counted_gather)
