...
async def test_run_datasmith_respects_observer_and_max_retries(monkeypatch):
    captured = {}
...
async def test_run_baseline_uses_datasmith_workload_mode(monkeypatch):
    captured = {}
    ...
    await rt.run_baseline(
        ...
        base_url="u",
        api_key=None,
        workload_mode="datasmith_strategy",
...
async def test_run_baseline_rejects_datasmith_workload_for_threaded(monkeypatch):
...
async def test_main_result_contains_runtime_metadata(monkeypatch, tmp_path):
    ...
    async def fake_run_datasmith(**kwargs):
        return {
            "method": "datasmith",
            "total_records": 1,
            "completed": 1,
    ...
    monkeypatch.setattr(rt, "run_datasmith", fake_run_datasmith)
    ...
    assert Path(captured["path"]).name.startswith("exp1_datasmith_vllm_")
